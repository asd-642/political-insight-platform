(function installHomepageImageGuard() {
  ...
  function pick(topic, seed, used) {
    const pool = pools[topic] || pools.policy;
    ...
    return pool[start] || pools.policy[0];
  }
  ...
  function apply() {
    ...
    cards.forEach((card, index) => {
      ...
      setImage(card.querySelector("img.thumb, img"), pick(topic, `${topic}|${title}|${index}`, used), pools.policy[index % pools.policy.length]);
    });
    ...
    if (featuredImg) {
      ...
      const topic = topicOf(text);
      setImage(featuredImg, pick(topic, `${topic}|featured|${text}`, new Set(used)), pools.policy[0]);
    }
    window.PolicyPulseImageGuard = {
      loaded: true,
  ...
  [100, 300, 700, 1200, 2500, 5000, 9000, 14000].forEach((delay) => setTimeout(apply, delay));
  ...
  const observer = new MutationObserver(() => {
    ...
    observer.timer = setTimeout(apply, 80);
  });
