...
function draftSummary(keyword, topicName) {
  return `${draftSubject(keyword, topicName)}先交代目前能確認的公開線索，再分開整理影響範圍、各方說法、資料缺口與接下來要看的文件。`;
}
  ...
  return [
    {
      heading: "為什麼這件事需要看",
      paragraphs: [
        `「${keyword}」近期出現在 ${topicName} 相關討論中。這篇先把能查到的公開線索、可能受影響的人，以及目前還說不清楚的地方分開整理。`,
        `目前來源包括：${sourceList}。這些資料只能作為起點，仍要回頭比對正式公告、議事紀錄、預算表或主管機關說明。`,
      ],
    },
    {
      heading: "目前能確認的線索",
      paragraphs: [
        ...
        "新聞摘要只能當作線索。文章會分開標示已知資訊與仍待確認的部分，避免把單一來源直接寫成完整結論。",
      ],
    },
    {
      heading: "政策影響會落在哪裡",
      paragraphs: [
        `「${keyword}」若進入公共政策討論，可能牽涉主管機關、地方執行單位、預算使用者與一般民眾。不同對象承受的成本與得到的好處不一樣，需要分開看。`,
        "若議題涉及補助、採購、稅制、交通、能源、居住或教育服務，重點不是宣示多漂亮，而是政策工具是否真的對應問題。",
      ],
    },
    {
      heading: "支持方說法怎麼讀",
      paragraphs: [
        support,
        "支持理由要拆成政策目的、預期效果、受益對象與可驗證指標。少了這些條件，讀者很難判斷政策是否真的有效。",
      ],
    },
    {
      heading: "疑慮方會追什麼",
      paragraphs: [
        concern,
        "保留意見不一定是反對政策，也可能是在追問成本、期程、責任歸屬與資訊公開。這些問題若沒有被回應，執行時容易出現落差。",
      ],
    },
    {
      heading: "還不能下結論的部分",
      paragraphs: [
        "目前若缺少正式公告、原始文件、議事紀錄、預算明細、統計表或主管機關書面回應，文章就不應急著替政策下定論。",
        "如果來源之間說法不一致，文章會保留各方版本，並把差異標示在時間線中，讓讀者看見資訊如何變化。",
      ],
    },
    {
      heading: "公開前後都要檢查",
      paragraphs: [
        "發布後仍要回頭確認標題是否過度下判斷、摘要是否把未證實內容寫成事實、來源是否足以支撐文章，以及分類是否正確。",
        "若文章涉及具名人物或機關責任，必須優先補上正式來源或公開紀錄，避免把未查證的指控寫成事實。",
      ],
    },
    {
      heading: "接下來要看哪裡",
      paragraphs: [
        next,
        "如果議題進入審查、發包、補助申請、地方執行或成效檢討階段，文章應更新時間點、責任單位與可量化指標，讓讀者看見政策是否真的落地。",
      ],
    },
  ];
}
...
function buildDraft({ keyword, topic, config, records, person = null }) {
  ...
  const support = `支持方說法可先看「${keyword}」是否能改善公共服務、補足制度缺口或提升資源使用效率，並要求提出可驗證的指標。`;
  const concern = `疑慮方通常會追問「${keyword}」相關政策的預算效率、執行落差、資訊透明度或責任歸屬，相關說法需要和公開紀錄對照。`;
  const next = "先看正式公告、議事紀錄、數據表、主管機關回應與各方正式說法。";
  ...
  const draft = {
    ...
    reviewChecklist: [
      "確認至少 2 個可查來源",
      "刪掉空泛套話，保留可驗證資訊",
      "補足事件背景、支持方、疑慮方與接下來要看的資料",
      "檢查是否有未查證指控、模糊歸因或過度判斷",
      "確認圖片來源或使用自動生成示意圖",
    ],
