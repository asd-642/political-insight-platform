(function installArticleSkillCopyCleanup() {
  const BAD_PUBLIC_PATTERNS =
    /(這篇文章聚焦|本文聚焦|來源摘要通常只呈現|目前抓到的來源包括|目前來源包括|Google News|支持方可能主張|後續追蹤方向|後續追蹤重點|需要補齊的資料|各方說法與執行進度)/;

  function cleanupArticleSkillCopy(value) {
    return String(value ?? "")
      .replace(
        /(?:這篇文章|本文)聚焦「([^」]+)」，先把近期來源中的相關標題與摘要整理成\s*([^。]+?)\s*議題脈絡。\s*它不是直接複製新聞，?[^。]*。?/g,
        (_, keyword, topic) => `「${String(keyword).trim()}」近期出現在${String(topic).trim()}相關討論中，本文先把公開線索、可能影響與仍待確認的資料分開整理。`,
      )
      .replace(
        /(?:這篇文章|本文)聚焦「([^」]+)」，先把近期來源中的相關標題與摘要整理成\s*([^。]+?)\s*議題脈絡。它不是直接複製新聞，而是把可追蹤的政策問題、影響對象與後續資料缺口先整理出來。?/g,
        (_, keyword, topic) => `「${String(keyword).trim()}」近期出現在${String(topic).trim()}相關討論中，本文先把公開線索、可能影響與仍待確認的資料分開整理。`,
      )
      .replace(/來源摘要通常只呈現事件表層，因此(?:文章|草稿)會把標題、摘要與關鍵字拆成待查核問題，而不是直接把外部報導當成完整結論。?/g, "新聞摘要只能當作線索，本文會分開標示已知資訊與仍待確認的部分。")
      .replace(/目前(?:抓到的)?來源包括：(?:Google News|新聞搜尋線索|[^。]+)。(?:系統|本文)會保留來源(?:標記|線索)，方便(?:後台檢查時判斷是否需要補|讀者對照)正式公告、議事紀錄(?:與|、預算表或)主管機關回應。?/g, "目前先以公開新聞線索作為入口，仍要回頭比對正式公告、議事紀錄、預算表或主管機關說明。")
      .replace(/目前來源包括：(?:Google News|新聞搜尋線索|[^。]+)。這些資料只能作為起點，仍要回頭比對正式公告、議事紀錄、預算表或主管機關說明。?/g, "目前先以公開新聞線索作為入口。這些資料只能當起點，仍要回頭比對正式公告、議事紀錄、預算表或主管機關說明。")
